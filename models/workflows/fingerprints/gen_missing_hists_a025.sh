#!/bin/bash
#SBATCH -J 025_missing_hist_gen
#SBATCH -N 2
#SBATCH --ntasks-per-node 48
#SBATCH -t 24:00:00
#SBATCH -p skx
#SBATCH -A TG-CHM250118
#SBATCH -o stdoutmsg_missing_hists
...
# Generate *.hist files missing from a025_fingerprints but present in other alpha
# folders. As of 2026-06-09 this is 21 frames (61 histogram files total):
#   frame_9, frame_80 (partial), frame_81-frame_99
#
# Cluster files (0.all-*-clusters.txt) already exist in each frame directory, so
# only the histogram step is needed (same as gen_hists.sh).
...
for dir in "${MISSING_FRAMES[@]}"; do
    ...
    # Use 4b histogram as completion marker (see debug-a000_fingerprints/gen_hists.sh).
    ...
    # Cluster generation already completed; histogram step only.
