#!/usr/bin/env bash
# Submit one SLURM job that sequentially fits all five 10% retention cuts
# (replicate 00, one per α) on a single skx allocation.
#
# skx-dev is limited to 2 hr and cannot finish 10% solves; this bundle uses skx.
#
#   cd models/pruned_models
#   bash submit_debug_pct010_bundle.sh
#   bash submit_debug_pct010_bundle.sh --dry-run

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
RUNS_DIR="${SCRIPT_DIR}/runs"
TEMPLATE="${SCRIPT_DIR}/templates/run_debug_pct010_bundle.cmd"
BUNDLE_CMD="${SCRIPT_DIR}/run_debug_pct010_bundle.cmd"
...
echo "Refreshing run dirs for pct010 rep00..."
python3 "${SCRIPT_DIR}/prepare_runs.py" --replicate 0 --debug-queue

pending=()
for run_dir in "${RUNS_DIR}"/a*_pct010_rep00/; do
    [[ -d "${run_dir}" ]] || continue
...
if [[ "${#pending[@]}" -eq 0 ]]; then
    echo "All five pct010 rep00 models already complete; nothing to submit."
    exit 0
fi

if [[ -f "${SCRIPT_DIR}/pct010_bundle_submitted.json" ]]; then
    bundle_id="$(python3 -c "import json; print(json.load(open('${SCRIPT_DIR}/pct010_bundle_submitted.json')).get('bundle_job',''))" 2>/dev/null || true)"
...
out = Path("${SCRIPT_DIR}") / "pct010_bundle_submitted.json"
...
echo "Submitted pct010 bundle: job=${bundle_id}"
echo "Monitor: squeue -j ${bundle_id}"
echo "Log:     ${SCRIPT_DIR}/pct010_bundle_${bundle_id}.out"
